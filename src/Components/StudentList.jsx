...
const StudentList = () => {
  ...
  const handleFilter = (mode) => {
    setListMode(mode);
    if (mode === "P") filteredStudents = students.filter(s => s.status === "present");
    else if (mode === "A") filteredStudents = students.filter(s => s.status === "absent");
    else if (mode === "U") filteredStudents = students.filter(s => s.status === "none");
    else filteredStudents = students;
  };

  if (listMode === "P") filteredStudents = students.filter(s => s.status === "present");
  else if (listMode === "A") filteredStudents = students.filter(s => s.status === "absent");
  else if (listMode === "U") filteredStudents = students.filter(s => s.status === "none");
  ...
  return (
    <div className="w-11/12 mx-auto my-5 bg-gradient-to-r from-blue-100 via-purple-100 to-indigo-100 p-5 rounded-2xl shadow-xl">
      ...
      <div className="max-h-[400px] overflow-y-auto mt-2">
        {isLoading && (
          <p className="text-center text-indigo-600 font-bold text-xl">Loading...</p>
        )}
        {isError && (
          <p className="text-center text-red-600 font-bold text-xl">Error: {error}</p>
        )}
        {!isLoading && !isError && filteredStudents?.length === 0 && (
          <p className="text-center text-indigo-700 font-bold text-xl">No students found</p>
        )}
